...
function stripCodeFence(content) {
  const fenced = content.match(/^```(?:json|text)?\s*([\s\S]*?)\s*```$/i);
...
function unwrapJsonContent(content) {
  ...
  try {
    ...
    for (const key of ["content", "reply", "message", "text"]) {
...
function cleanupGeneratedText(content) {
  return content
    .trim()
    .replace(/^([“”"'`]+)|([“”"'`]+)$/g, "")
...
function normalizeGeneratedContent(value) {
  ...
  content = cleanupGeneratedText(unwrapJsonContent(cleanupGeneratedText(stripCodeFence(content))));
  if (!content) return "";
